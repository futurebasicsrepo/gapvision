...
| **Outbound email** | Pointed at an SMTP provider, not at Gmail — Google does not offer app passwords to Workspace accounts, and `kr@cuesea.ai` is one. `CUE_SMTP_HOST`, `CUE_SMTP_PORT`, `CUE_SMTP_USER` and `CUE_MAIL_FROM` are set on the ai-service; **`CUE_SMTP_PASSWORD` is not**, and until it is the provider stays `console` — invites and resets go to the service log rather than an inbox, and Health says so rather than showing green. Partial configuration is safe by design: `provider()` requires all three of host, user and password. Health → **Send a test** attempts one real send to the caller's own address and names which of the six ways SMTP fails actually happened, because a rejected key looks identical to an accepted one until something is sent. |
