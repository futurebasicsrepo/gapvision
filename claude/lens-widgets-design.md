...
| **PROMOS** | Current promotions. **Kyle, 17 Aug 2026: the source is a PDF or a live feed at a URL — a marketing asset in Shopify, for example — not hand-typed copy.** That is a better answer than the one this table originally offered, and it changes the work: the widget is downstream of an *ingest* rather than of a text field. A PDF is the harder half (extract, then decide what three lines of a promotion actually are on 576px), a URL feed is the easier one, and both want the same shape underneath: a per-tenant list of promotions with a validity window, refreshed on a timer, cached so a shop's wifi is never in the path of a cue. | Medium. The lens half stays trivial; the ingest and the "what is a promotion, in three lines" question are the work. |
